...
class Solver:
    ...
    def load_state(self, state_string):
        ...
        if invalid:
            raise ValueError(f"Invalid characters: {sorted(invalid)}. Only U R F D L B are allowed.")

        wrong = {f: state_string.count(f) for f in VALID_FACES if state_string.count(f) != 9} #check for correct face counts
        if wrong:
            raise ValueError(f"Each face letter must appear exactly 9 times. Incorrect counts: {wrong}")
    ...
    def select_algorithm(self, name):
        ...
        if name not in AlgorithmSelector.ALGORITHMS:
            raise ValueError(f"Unknown algorithm '{name}'. Supported: {', '.join(sorted(AlgorithmSelector.ALGORITHMS))}.")
        self._alg_name  = name
        self._algorithm = None
